function [infection_mat,  detection_mat, recovered_mat, q_idx, fail_idx, q_ball_close, q_ball_check, enforce]=...
...
%list of nodes to track for initial quarantine
q_ball_check=nearest(obs_graph, seed, q_radius-1);
q_ball_close=nearest(obs_graph, seed, q_radius);
q_ball_close=[q_ball_close; seed];
...
for tt=2:T
   ...
   % initial trigger
   if sum(detection_mat(q_ball_check,tt))>=x  && initial_quarantine_idx==0
    ...
    if fail_idx==0
        farthest=max(max(distances(obs_graph, seed, find(infection_mat(:,tt)))));
